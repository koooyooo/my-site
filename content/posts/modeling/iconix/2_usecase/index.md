...
##### 2. 既存モデルの修正
更に、当初のドメインモデルのユースケースが見えたことで既存のドメインモデルの**名称に違和感**が出る場合もあります。その場合は既存のドメインモデルに修正を入れます。

> 以上の過程でドメインモデル上に不足していたモデルが現れ、既出のモデルはユースケース上で検証・最適化されます。この先の、ロバストネス分析・ユースケース記述といった工程でもモデルは随時磨かれてゆきます。こうした過程が存在するから、ドメイン分析は数時間で切り上げることが出来るわけです。

#### ユースケース記述
ユースケース分析では、個々のユースケースを図として抽出出来たら、本来ユースケース記述という工程に入ります。これは従来の開発プロセスに当てはめると**要件定義**の後に要件毎の**詳細設計**を行うイメージとなります。しかし ICONIXの工程ではこの上位の**基本設計**をドメインモデルと結びつけながら行う「**ロバストネス分析**」という工程が存在します。

今後、ユースケース記述の前段となるロバストネス分析も見てゆく予定です

### まとめ

今回はアクターの分析から始まりました。観点と抽象度に気をつけながらアクターを定義し、その後アクターの汎化も行いました。次にユースケースを段階的に定義し、システム境界や関連線の詳細化を行いました。その過程で随時ドメインモデルには更新を行い、静的モデルを洗練させると同時に、動的モデルの要件定義となるユースケース図を完成させました。

次回はロバストネス分析を通じてドメインモデルとユースケースを更に詳細化し、システム化への道筋を見つけてゆきたいと思います。
